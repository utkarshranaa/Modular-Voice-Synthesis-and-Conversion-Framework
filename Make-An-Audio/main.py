...
# batchloader outputshape should be (b,h,w,c) and it will be permuted to (b,c,h,w) in autoencoder.get_input()
...
# will not load ckpt, just set directories for the experiment
    ...
    def _log(self, pl_module, images, batch_idx, split):
        for k in images:
            ...
            pl_module.logger.experiment.add_figure(
                tag, fig, global_step=pl_module.global_step)
        ...
        if (self.check_frequency(check_idx) and  # batch_idx % self.batch_freq == 0
            ...
            with torch.no_grad():  # 这里会调用ddpm中的log_images
                images = pl_module.log_images(
                    batch, split=split, **self.log_images_kwargs)  # images is a dict
...
class AudioLogger(ImageLogger):
    def __init__(self, batch_frequency, max_images, increase_log_steps=True, melvmin=0, melvmax=1, disabled=False, log_on_batch_idx=False, log_first_step=False,
        ...
        super().__init__(batch_frequency, max_images,  increase_log_steps,  disabled,
                         log_on_batch_idx, log_first_step, melvmin, melvmax, log_images_kwargs)
    ...
    def _visualize_attention(self, attention, scale_by_prior=True):
        if scale_by_prior:
            ...
            attention_uniform_prior = 1 / \
                torch.arange(1, T+1).view(1, T, 1).repeat(B, 1, T)
            attention_uniform_prior = attention_uniform_prior.tril().view(
                B, 1, T, T).to(attention.device)
        ...
        for i, spec in enumerate(specs):
            ...
            if pl_module is not None:
                pl_module.logger.experiment.add_audio(
                    f'{tag}_{i}', wave, global_step, self.sample_rate)
            # in case we would like to save it on disk
            if save_rec_path is not None:
                soundfile.write(save_rec_path, wave.squeeze(
                    0).numpy(), self.sample_rate, 'FLOAT')
    ...
    def log_local(self, save_dir, split, images,
        ...
        for k in images:
            ...
            if self.for_specs:
                ...
                if k not in ['conditioning', 'conditioning_rec', 'att_nopix', 'att_half', 'att_det']:
                    self._log_rec_audio(
                        images[k], tag, global_step, save_rec_path=path)
...
class CUDACallback(Callback):
    ...
    # ,outputs： outputs positional argument has been removed in the later pytorch-lighning version。
...
if __name__ == "__main__":
    ...
    try:
        ...
        # default to ddp
        # "ddp" # "ddp_find_unused_parameters_false"
        ...
        if not opt.no_test and not trainer.interrupted:
            # just test the ckeckpoint, without training
